...
echo "[*] Cloning AnyKernel3 into workspace (branch: kona)..."
rm -rf anykernel
git clone https://github.com/AstideLabs/AnyKernel3 -b kona --single-branch --depth=1 anykernel || { echo "[!] Failed to clone AnyKernel3"; exit 1; }

if [ -f "anykernel/anykernel.sh" ]; then
    sed -i "s/^device\.name1=.*/device.name1=${DEVICE_NAME}/" anykernel/anykernel.sh
    echo "[*] AnyKernel3 device.name1 set to: ${DEVICE_NAME}"
fi
